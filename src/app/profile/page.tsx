...
import { Loader2, CheckCircle, ChevronLeft, User, Lock, Calendar, QrCode } from 'lucide-react';
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(240px, 100%), 1fr))", gap: "24px" }}>
          ...
          <div className="rounded-2xl border p-5" style={{ background: 'var(--bg2)', borderColor: 'var(--border)', height: 'fit-content' }}>
            <div className="text-center mb-5">
              ...
              {form.companyName && (
                <p className="text-xs mt-0.5" style={{ color: 'var(--accent)' }}>by {form.firstName} {form.lastName}</p>
              )}
              <span className="inline-block text-xs px-2 py-0.5 rounded mt-1"
                style={{ background: 'var(--bg3)', color: 'var(--accent)', border: '0.5px solid var(--border)' }}>
                {user?.role}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-1">
              {[
                ...
                <button key={tab.key} onClick={() => { setActiveTab(tab.key as any); setError(''); setSuccess(''); }}
                  ...
                  style={{
                    ...
                    color: activeTab === tab.key ? 'var(--text-bright)' : 'var(--accent)',
                  }}>
                  {tab.icon} {tab.label}
                </button>
              ))}
              {isOrganizer && (
                <>
                  <Link href="/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold"
                    style={{ color: 'var(--accent)' }}>
                    <Calendar size={14} /> My Events
                  </Link>
                  <Link href="/scanner" className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold"
                    style={{ color: 'var(--accent)' }}>
              ...
              <form onSubmit={handleSave} className="space-y-5">
                ...
                <div className="flex items-center gap-5 pb-5 border-b" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <div>
                    ...
                    <p className="text-xs mb-3" style={{ color: 'var(--accent)' }}>
                      {avatarUrl ? 'Your photo is visible on your public profile and event listings.' : 'Upload a photo to personalize your profile. JPG or PNG. Max 2MB.'}
                    </p>
                    <div className="flex gap-2">
                      <label className="flex items-center gap-2 font-bold text-xs px-4 py-2 rounded-lg cursor-pointer"
                      ...
                      {avatarUrl && (
                        <button type="button" onClick={handleAvatarRemove}
                          className="font-bold text-xs px-4 py-2 rounded-lg border"
                          style={{ borderColor: 'rgba(224,85,85,0.3)', color: '#e05555', background: 'transparent' }}>
                          Remove
                        </button>
                      )}
                    </div>
                ...
                <div>
                  ...
                  <input type="email" value={user?.email || ''} disabled
                    className="w-full px-4 py-3 rounded-lg text-sm outline-none border opacity-50" style={inputStyle} />
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <input type="text" value={form.companyName} onChange={e => setForm({ ...form, companyName: e.target.value })}
                        placeholder="e.g. Lagos Live Events" className="w-full px-4 py-3 rounded-lg text-sm outline-none border" style={inputStyle} />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" value={form.city} onChange={e => setForm({ ...form, city: e.target.value })}
                          placeholder="e.g. Lagos" className="w-full px-4 py-3 rounded-lg text-sm outline-none border" style={inputStyle} />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <textarea value={form.bio} onChange={e => setForm({ ...form, bio: e.target.value })}
                        placeholder="Tell attendees about you or your organization..." rows={3}
                ...
                <div className="flex gap-3 pt-2">
                  <button type="submit" disabled={saving}
                    className="font-bold text-sm px-6 py-3 rounded-xl flex items-center gap-2"
                    style={{ background: 'var(--text-bright)', color: 'var(--bg)' }}>
                    {saving ? <><Loader2 size={14} className="animate-spin" /> Saving...</> : 'Save changes'}
                  </button>
                  <button type="button" onClick={() => router.back()}
                    className="font-bold text-sm px-6 py-3 rounded-xl border"
